...
movement_mapping = {'backhand': 0, 'forehand': 1, 'serve': 2, 'b_slice': 3, 'b_volley': 4,
                    'f_volley': 5, 'smash': 6}

HEIGHT, WEIGHT = 135, 240

train_frames = np.empty((0, 32, HEIGHT, WEIGHT, 3), dtype=np.float32)
train_classes = np.empty(0, dtype=np.int32)

test_frames = np.empty((0, 32, HEIGHT, WEIGHT, 3), dtype=np.float32)
test_classes = np.empty(0, dtype=np.int32)

val_frames = np.empty((0, 32, HEIGHT, WEIGHT, 3), dtype=np.float32)
    ...
    for video in videos:
      ...
      for frame_number in frame_numbers:
        ...
        if ret:
          ...
          frame = run_inference(HEIGHT, WEIGHT, frame)
...
np.save('skeleton32_train_frames.npy', train_frames)
np.save('skeleton32_train_classes.npy', train_classes)

np.save('skeleton32_test_frames.npy', test_frames)
np.save('skeleton32_test_classes.npy', test_classes)

np.save('skeleton32_val_frames.npy', val_frames)
np.save('skeleton32_val_classes.npy', val_classes)